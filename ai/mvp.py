...
def summarize_key_takeaways(doc: str) -> str:
    """
    calls LLM to summarize the Key Takeaways section of the response.

    :param doc: the response for which to summarize the Key Takeaways section.

    Returns a two-sentence summary of Key Takeaways.
    """
    prompt = PromptTemplate(
        template=summarize_key_takeaways_template, input_variables=["doc"]
    )

    chain = {"doc": RunnablePassthrough()} | prompt | get_LLM() | StrOutputParser()

    doc_doc = Document(doc)

    return chain.invoke(doc_doc)
