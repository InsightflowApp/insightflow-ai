...
def connect_db():
    ...
    if client is None:
        ...
        CONNECTION_STRING = (
            ...
            + "@insightflow0.bn2saqf.mongodb.net/"
            + "?retryWrites=true&w=majority&appName=Insightflow0"
        )
...
def update_doc(collection, filters, update_dict, db=DB):
    return client[db][collection].update_one(
        filters, {"$set": update_dict}, upsert=True
    )
...
def fetch_all_with_filter(collection, filt=None, db=DB):
    ret = []
    if filt is None:
        filt = {}
